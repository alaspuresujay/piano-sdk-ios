...
enum CXConsentVersion : NSInteger;
...
@property (nonatomic) enum CXConsentVersion consentVersion;
/// Cxense site-specific persistent cookie
...
/// Allowed usage of user-agent and other device-specific data.
  CXConsentOptionDeviceAllowed = 5,
/// Allowed usage of geolocation with page view events
  CXConsentOptionGeoAllowed = 6,
};

typedef SWIFT_ENUM_NAMED(NSInteger, CXConsentVersion, "ConsentVersion", open) {
  CXConsentVersionV1 = 0,
  CXConsentVersionV2 = 1,
};
...
///
+ (void)forceReportEvent:(CXEvent * _Nonnull)event callback:(void (^ _Nullable)(BOOL, NSError * _Nullable))callback;
...
/// \param query Event query to filter events..
///
+ (void)removeEventWithQuery:(Query * _Nonnull)query;
...
/// Provide persistent cookie from configuration.
///
/// returns:
/// Cxense site-specific persistent cookie.
+ (NSString * _Nonnull)getPersistentCookie SWIFT_WARN_UNUSED_RESULT;
/// Replace persistent cookie in configuration.
/// \param persistentCookie Cxense site-specific persistent cookie.
///
+ (BOOL)replacePersistentCookieWithPersistentCookie:(NSString * _Nonnull)persistentCookie error:(NSError * _Nullable * _Nullable)error;
/// Clear persistent cookie in configuration and set to default.
/// \param persistentCookie Cxense site-specific persistent cookie.
///
+ (BOOL)clearPersistentCookieAndReturnError:(NSError * _Nullable * _Nullable)error;
...
/// Lock event.
/// \param query Event query for lock.
///
/// \param duration Lock duration in seconds.
///
+ (id <Lockable> _Nonnull)lockEventWithQuery:(Query * _Nonnull)query duration:(int64_t)duration SWIFT_WARN_UNUSED_RESULT;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
...
SWIFT_CLASS("_TtC9CxenseSDK19DelayEventProcessor")
@interface DelayEventProcessor : NSObject
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
...
@interface CXEvent (SWIFT_EXTENSION(CxenseSDK))
@end



/// Lockable object.
SWIFT_PROTOCOL("_TtP9CxenseSDK8Lockable_")
@protocol Lockable
/// Unlock object.
- (void)unlock;
@end
...
@interface CXPageViewEvent (SWIFT_EXTENSION(CxenseSDK))
@end
...
/// returns:
/// builder’s instance
- (nonnull instancetype)setPersistentCookie:(NSString * _Nonnull)ckp SWIFT_WARN_UNUSED_RESULT SWIFT_DEPRECATED_MSG("Use Cxense configuration property persistentCookie");
...
SWIFT_CLASS_NAMED("ServerError")
@interface CXServerError : NSObject
